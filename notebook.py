...
for i, feature in enumerate(features):
    ...
    sns.histplot(training[feature], ax=ax)
...
sns.heatmap(training[features].corr(), ax=ax, cmap='coolwarm')
...
# Plot each genre separately with different colors
for genre in genres:
    sns.kdeplot(
        data=training[training['label'] == genre][feature],
        label=genre,
        ax=ax
    )
...
if github:
    plt.savefig('images/tempo_distribution.png')
else:
    plt.show()
...
if github:
    plt.savefig('images/chroma_stft_distribution.png')
else:
    plt.show()
...
if github:
    plt.savefig('images/rmse_distribution.png')
else:
    plt.show()
...
if github:
    plt.savefig('images/spectral_centroid_distribution.png')
else:
    plt.show()
...
ax = fig.add_subplot(gs[0])
sns.heatmap(training[mfcc].corr(), ax=ax, cmap='coolwarm')
ax.set_title('MFCC index correlation heatmap')

if github:
    plt.savefig('images/mfcc_correlation_heatmap.png')
else:
    plt.show()
...
gs = GridSpec(round(len(to_plot)/2), 2)
...
fig.suptitle('MFCC index distribution')

if github:
    plt.savefig('images/mfcc_index_distribution.png')
else:
    plt.show()
...
for feature in to_plot:
    ...
    ax = fig.add_subplot(gs[0])
    sns.histplot(training[feature].loc[training['label'] == genre], ax=ax, kde=True)
